...
pretty_assertion_attributes = function(assertion_attributes) {
  ...
  return _.chain(assertion_attributes).entries().filter(function(arg) {
    var k, v;
    k = arg[0], v = arg[1];
...
parse_authn_response = function(saml_response, sp_private_keys, idp_certificates, allow_unencrypted, ignore_signature, require_session_index, ignore_timing, require_notonorafter, require_notbefore, require_onetimeuse, notbefore_skew, sp_audience, cb) {
  ...
  return async.waterfall([
    function(cb_wf) {
      ...
      var assertion, encryptedAssertion, ex, i, l, len1, len2, m, ref2, saml_response_str, sd, signed_data, signed_dom;
      debug(result);
      if (ignore_signature) {
      ...
      var attribute, attributes, audience_restriction, audiences, condition, conditions, l, len1, nb, nooa, otu, validAudience;
      conditions = decrypted_assertion.getElementsByTagNameNS(XMLNS.SAML, "Conditions")[0];
      if (conditions != null) {
        if (!ignore_timing) {
          attributes = _.map(conditions.attributes, function(v, k) {
            return {
          ...
          otu = _.size(_.filter(conditions.childNodes, function(v, k) {
            return v.nodeName.toLowerCase() === "onetimeuse";
          }));
          ...
          if (require_notonorafter && require_notbefore && (!nooa || !nb)) {
        ...
        if (require_notonorafter && require_notbefore) {
        ...
        return function(response_buffer, cb_wf) {
          ...
          switch (false) {
            case saml_response.getElementsByTagNameNS(XMLNS.SAMLP, "Response").length !== 1:
              ...
              return parse_authn_response(saml_response, [_this.private_key].concat(_this.alt_private_keys), identity_provider.certificates, options.allow_unencrypted_assertion, options.ignore_signature, options.require_session_index, options.ignore_timing, options.require_notonorafter, options.require_notbefore, options.require_onetimeuse, options.notbefore_skew, options.audience, cb_wf);
